...
constexpr uint32_t MAX_EXTRAPOLATION_TIMES = 3;

// 构造函数：预开辟空间，空间上构造目标
TrackerManager::TrackerManager(uint32_t track_size, uint32_t point_size) : next_track_id_(1), max_point_size(point_size)
{

    // 预分配内存，提高性能
    buffer_pool_.reserve(track_size);
    track_id_to_pool_index_.reserve(track_size);   // rehash会给creat任务带来约一倍延时
...
uint32_t TrackerManager::createTrack()
{
    ...
    uint32_t pool_index = free_slots_.back();
...
bool TrackerManager::deleteTrack(uint32_t track_id)
{
...
bool TrackerManager::push_track_point(uint32_t track_id, TrackPoint point)
{
    // 搜索目标航迹
    auto it = track_id_to_pool_index_.find(track_id);
    ...
    // 数据处理
    track.header.point_num = static_cast<uint32_t>(track.data.size()); // 更新点迹数量
...
bool TrackerManager::merge_tracks(uint32_t source_track_id, uint32_t target_track_id)
{
    // 搜索目标航迹
    ...
    // 获取航迹
    uint32_t target_pool_index = target_it->second;
    TrackerContainer &target_track = buffer_pool_[target_pool_index];
    uint32_t source_pool_index = source_it->second;
    TrackerContainer &source_track = buffer_pool_[source_pool_index];

    // 异常处理
    uint32_t target_size = static_cast<uint32_t>(target_track.data.size());
    uint32_t source_size = static_cast<uint32_t>(source_track.data.size());
...
size_t TrackerManager::pack_track(char *buffer, uint32_t track_id)
{
    // ID号
    auto it = track_id_to_pool_index_.find(track_id);
    ...
    uint32_t max_points = static_cast<uint32_t>(track.data.size());
...
void TrackerManager::clearAll()
{
    ...
    // 重建空闲槽位列表
    for (uint32_t i = 0; i < buffer_pool_.size(); ++i)
    {
        free_slots_.push_back(i);
    }
