...
4. - 泛型环形存储容器： **latestKbuffer\<T\>**
   ...
   - 可视化组件：**TrackerVisualizer**
     - 读取航迹结构，可视化，一切从简(当单片机画)
     - 日志库也算可视化，加进去
     - 整个流水线结构的可视化放到JS去实现，绘制所有信息
   - 通信组件：**TrackerComm**
...
3. 基础可视化插件：
   ...
   - 航迹线绘制：红色线条，新点透明度高，历史点透明度低
   - 航迹号标注：在最新点位置显示航迹 ID
   - 经纬度坐标转换：自动映射到图像坐标系
   - 实时刷新：支持动态更新显示
      | 数据规模 | 总点数 | 平均绘制时间 | 刷新频率 |
      |---------|--------|-------------|----------|
      | 1万点 | 10,000 | 14.67 ms | 68.2 FPS | 
      | 5万点 | 50,000 | 47.56 ms | 21.0 FPS | 
      | 10万点 | 100,000 | 89.53 ms | 11.2 FPS | 
      | 25万点 | 250,000 | 75.17 ms | 13.3 FPS | 
      | 50万点 | 500,000 | 148.17 ms | 6.7 FPS |
